...
typedef PRECISION_T precision_t;  // what type to use for most major calculations, the more precise the deeper you can
								  // zoom but the slower it will most likely be.
typedef long double normalized_t;  // which type to use for normalized iteration output, more precise means more
								   // smoothness but less speed.
...
int in_mand_set_orbit(std::complex<long double> c) {  // this is also unused, go down one function
	...
	while (true) {
		i++;
		long double xtemp = z.real() * z.real() - z.imag() * z.imag() + c.real();  // z = std::pow(z, d) + c;
		z.imag(2 * z.real() * z.imag() + c.imag());
		z.real(xtemp);

		xtemp = z.real() * z.real() - z.imag() * z.imag() + c.real();  // z = std::pow(z, d) + c;
...
int in_mand_set2(std::complex<precision_t> c) {
	...
	while (i) {
		i--;
		precision_t xtemp = z.real() * z.real() - z.imag() * z.imag() + c.real();  // z = std::pow(z, d) + c;
		z.imag(2 * z.real() * z.imag() + c.imag());
		z.real(xtemp);
...
int auto_iters() {
	long double f =
		(long double)sqrt(0.001 + 2 * std::min(abs((-aspect_zoom.real() + x_pos) - (aspect_zoom.real() + x_pos)),
...
int screenshot(SDL_Renderer* renderer, SDL_Window* window) {
	...
	filenameSS << std::setprecision(std::numeric_limits<long double>::digits10 + 2) << "screenshots/"
			   << set_names[set] << "_" << x_pos << "," << y_pos << "," << zoom << "," << std::to_string(iterations)
			   << "," << std::to_string(r) << "," + std::to_string(g) << "," << std::to_string(b) + "-"
			   << std::to_string(win_w) << "x" << std::to_string(win_h) << ".bmp";
...
std::vector<normalized_t> compute_escape_time(unsigned int starty,
											  unsigned int endy,  // TODO: make this NOT USE FLOAT VECTIR
											  unsigned int ix,
											  int win_w,
											  int win_h,
											  unsigned int iz) {
	...
	for (double x = ix; x < win_w; x += iz) {
		for (double y = starty; y < endy; y += 1) {
			iters = 0;
			precision_t x_point = naive_lerp(-aspect_zoom.real() + x_pos, aspect_zoom.real() + x_pos, x / win_w);
			precision_t y_point = naive_lerp(-aspect_zoom.imag() + y_pos, aspect_zoom.imag() + y_pos, y / win_h);
			switch (set) {
				case 0:
...
std::vector<normalized_t> compute_normalized(unsigned int starty,
											 unsigned int endy,
											 unsigned int ix,
											 int win_w,
											 int win_h,
	...
	for (double x = ix; x < win_w; x += iz) {
		for (double y = starty; y < endy; y += 1) {
			iters = 0;
			precision_t x_point =
				naive_lerp(-aspect_zoom.real() + x_pos, aspect_zoom.real() + x_pos, (precision_t)x / win_w);
			precision_t y_point =
				naive_lerp(-aspect_zoom.imag() + y_pos, aspect_zoom.imag() + y_pos, (precision_t)y / win_h);
			switch (set) {
				case 0:
...
int render(SDL_Renderer* renderer, SDL_Window* window) {
	...
	if (set <= et_sets) {
...
#ifndef PRECISION_FIX
		int xc = std::fpclassify(x_pos);  // NOTE: custom precision no likey
		int yc = std::fpclassify(y_pos);
		int zoomc = std::fpclassify(zoom);
		if (xc == FP_NAN || xc == FP_INFINITE || yc == FP_NAN || yc == FP_INFINITE || zoomc == FP_NAN ||
			zoomc == FP_INFINITE) {
			x_pos = 0;
			y_pos = 0;
			zoom = 2;
		...
		double ratioA = double(abs((-zoom + x_pos) - (zoom + x_pos)) / abs((-zoom + y_pos) - (zoom + y_pos)));
		double ratioB = double(win_w) / win_h;
		...
		std::stringstream log;
		log << std::setprecision(std::numeric_limits<long double>::digits10 + 2) << x_pos << "," << y_pos << ":"
			<< zoom << " (" << iterations << ")";
		...
		for (unsigned int ie = 0; ie < 4; ie += 1) {
			...
			for (unsigned int i = 0; i < threads - 1; i += 1) {
				if (normalized) {
					futures.push_back(std::async(std::launch::async, compute_normalized, chunksize * i,
												 chunksize * (i + 1), ix, win_w, win_h, 4));
				} else {
					futures.push_back(std::async(std::launch::async, compute_escape_time, chunksize * i,
												 chunksize * (i + 1), ix, win_w, win_h, 4));
				}
			}
			...
			if (normalized) {
				lastrender = compute_normalized(chunksize * (threads - 1), win_h, ix, win_w, win_h, 4);
			} else {
				lastrender = compute_escape_time(chunksize * (threads - 1), win_h, ix, win_w, win_h, 4);
			}
			...
			for (unsigned int i = 0; i < threads - 1; i += 1) {
				...
				renderedvec.insert(renderedvec.end(), temprendered.begin(), temprendered.end());
			}

			renderedvec.insert(renderedvec.end(), lastrender.begin(), lastrender.end());
		...
		char* windowtitle = str_to_char("Cool Fractal Viewer (" + std::to_string(diff.count()) + " s)");
		...
		*/
		...
		SDL_SetWindowTitle(window, "Cool Fractal Viewer (Rendering)");
		std::chrono::steady_clock::time_point start =
			std::chrono::steady_clock::now();  // TODO: fix biggie big: NOTE: pressing random key (like w or some
											   // shit) while rendering will make the render cease to render! check if
											   // the key is actually important first.
...
int handle_events(SDL_Event event, SDL_Renderer* renderer, SDL_Window* window) {
	switch (event.type) {
		...
		case SDL_MOUSEBUTTONDOWN:
			if (event.button.button == SDL_BUTTON_LEFT) {  // center screen on mouse cursor upon left-click
				int win_w, win_h;
				SDL_GetWindowSize(window, &win_w, &win_h);
		...
		case SDL_MOUSEWHEEL:
			if (event.wheel.y > 0) {  // zoom in on mouse cursor on scroll up
				...
				double ratioA = double(
					abs((-zoom + x_pos) - (zoom + x_pos)) /
					abs((-zoom + y_pos) -
						(zoom + y_pos)));  // maybe moving around the casting will make it faster? static cast maybe?
				double ratioB = double(win_w) / win_h;
				...
				render(renderer, window);
			} else if (event.wheel.y < 0) {	 // zoom out away from mouse cursor on scroll down
				int win_w, win_h;
				SDL_GetWindowSize(window, &win_w, &win_h);
...
extern "C"
	...
	dtest1 << std::setprecision(std::numeric_limits<long double>::digits10 + 2) << d;
	std::stringstream dtest2;
	dtest2 << std::setprecision(std::numeric_limits<long double>::digits10 + 2) << int(d);
