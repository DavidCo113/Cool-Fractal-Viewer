...
inline int render_triangle(SDL_Renderer *renderer, std::complex<double> pos1,
                           std::complex<double> pos2, std::complex<double> pos3,
                           int r, int g, int b) {
  SDL_Vertex vert[3];
  // center
...
render_invert_triangle(SDL_Renderer *renderer, std::complex<double> pos1,
                       std::complex<double> pos2, std::complex<double> pos3) {
  double x2 = pos2.real();
  double y2 = pos1.imag();
...
render_sierp_iter(SDL_Renderer *renderer,
...
inline SDL_Texture *render_sierpinski(SDL_Renderer *renderer,
                                      SDL_Window *window, int iterations,
  ...
  if (iterations >= 1) {
    render_triangle(
        renderer,
        std::complex<double>(double(win_w) / 2, double(win_h) / 2 - scale / 2),
        std::complex<double>(double(win_w) / 2 - scale / 2,
                             double(win_h) / 2 + scale / 2),
        std::complex<double>(double(win_w) / 2 + scale / 2,
                             double(win_h) / 2 + scale / 2),
        0, 0, 0);
    if (iterations >= 2) {
      std::vector<std::complex<double>> inv = render_invert_triangle(
          renderer,
          std::complex<double>(double(win_w) / 2,
                               double(win_h) / 2 - scale / 2),
          std::complex<double>(double(win_w) / 2 - scale / 2,
                               double(win_h) / 2 + scale / 2),
          std::complex<double>(double(win_w) / 2 + scale / 2,
                               double(win_h) / 2 + scale / 2));
      if (iterations >= 3) {
        std::vector<std::vector<std::complex<double>>> invs =
            render_sierp_iter(renderer, inv);
  ...
  for (int i = 0; i < iterations - 3; i++) {
    while (positions.size()) {
      std::vector<std::vector<std::complex<double>>> out =
          render_sierp_iter(renderer, positions.at(0));
